function doCapture(cb) {
	...
	showResult.innerHTML = "capturing...";
	showPicture.src = "images/qr.png";
...
//		showPicture.src = dataURL;
		showResult.innerHTML = "processing";

		qrcode.callback = function(data) {
			showResult.innerHTML = "done!";
			if(!data.indexOf("http")) {
				showResult.className = "link";
			}
			showResult.onclick = null;
			if(data) {
				...
				showResult.onclick = function () {
					var openURL = new MozActivity({
						name: "view",
						data: {
							type: "url", // Possibly text/html in future
							url: data
						}
					});
				}
			} else {
				showResult.innerHTML = "no data found.";
			}
		};
...
//		}, 100);
	...
	activity.onerror = function() {
		if(typeof activity.error === "string") {
			showResult.innerHTML = activity.error;
		} else {
